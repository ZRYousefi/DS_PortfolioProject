...
def clean_tbl_name(filename):
    ...
    # r"(" means this is just a raw string here
...
def connect_to_db(host, dbname, user, password):
    conn_string = "host=%s dbname=%s user=%s password=%s" % (host, dbname, user, password) # port?
    try: 
...
def upload_to_db(host, dbname, user, password, tbl_name, col_str, file, dataframe, dataframe_columns):
    ...
    # 2. open the csv file, save it as an object
    my_file = open(file)
